import { NextRequest, NextResponse } from "next/server";
import OpenAI from "openai";

const openai = new OpenAI({
    apiKey: process.env.OPENAI_API_KEY,
});

export async function POST(request: NextRequest) {
    try {
        console.log("[AI Resource Recommend] API 호출 시작");
        const { activity, category } = await request.json();
        console.log("[AI Resource Recommend] 요청 데이터:", { activity, category });

        if (!activity) {
            return NextResponse.json(
                { error: "Activity is required" },
                { status: 400 }
            );
        }

        // Get current context for personalized messages
        const now = new Date();
        const hour = now.getHours();
        const currentSeason = now.getMonth() >= 11 || now.getMonth() <= 1 ? "겨울" :
                             now.getMonth() >= 2 && now.getMonth() <= 4 ? "봄" :
                             now.getMonth() >= 5 && now.getMonth() <= 7 ? "여름" : "가을";
        const timeOfDay = hour < 12 ? "오전" : hour < 18 ? "오후" : "저녁";

        // Generate professional assistant messages with value
        const prompt = `사용자가 "${activity}" 일정을 추가했습니다.

현재 상황:
- 시간대: ${timeOfDay} ${hour}시
- 계절: ${currentSeason}
- 활동 카테고리: ${category}

전문 AI 비서로서 다음 원칙에 따라 메시지를 작성하세요:

**핵심 원칙**
- 존댓말만 사용 (반말 절대 금지)
- 필요한 정보만 정확하게 전달
- 실질적인 도움이 되는 리소스나 행동 제시
- 각 활동마다 완전히 다른 접근 방식

**활동별 메시지 스타일**

📚 **독서/학습** - 리소스 제공 중심
"[활동] 일정을 추가하셨네요. [구체적 리소스 1-2개: 교보문고/YES24/알라딘/인강 플랫폼]. [추가 지원 제안 질문]"
예: "독서 시간이시네요 📚 교보문고나 YES24에서 '린 스타트업'을 검색하시면 MVP 개념을 잘 설명한 책을 찾으실 수 있어요. 다른 추천 도서가 필요하시면 말씀해주세요."

💼 **회의/업무** - 준비 체크 + 지원
"[회의/업무] 준비 시간입니다. [핵심 체크포인트]. [자료 지원 제안]"
예: "회의 준비 시간입니다. 발표 자료와 참고 문서 재점검하셨나요? 회의 주제 관련 추가 자료가 필요하시면 구체적인 내용을 알려주세요."

🚀 **프로젝트/코딩** - 실용 리소스 + 조언
"[프로젝트] 작업 시작이시군요. [구체적 리소스/검색어]. [핵심 조언 1가지]"
예: "AI MVP 작업이시군요. GitHub에서 'ai mvp boilerplate'나 'fastapi langchain'으로 검색하시면 참고할 템플릿들이 많아요. 핵심 기능 하나만 먼저 완성하시고 확장하시길 추천드려요."

💪 **운동** - 실용 팁 + 리소스
"[운동] 시간입니다! [계절/날씨 고려 팁]. [유튜브/앱 추천]"
예: "운동 시간입니다 💪 겨울이니 충분히 워밍업하세요. 유튜브에서 '초보 홈트레이닝'을 검색하시면 따라하기 좋은 루틴을 찾으실 수 있어요."

☕ **휴식/사교** - 장소/환경 제안 중심
"[활동]이시군요. [장소/환경 찾기 팁]. [실용 질문/제안]"
예: "커피챗 시간이시네요 ☕ 네이버 지도에서 '조용한 카페'로 검색하시면 대화하기 좋은 곳들이 나와요. 미리 대화 주제를 정해두시면 더 알찬 시간이 될 거예요."

🍽️ **식사** - 메뉴 제안 + 장소
"[식사] 시간이에요. [시간대/계절 고려 메뉴 추천]. [맛집 찾기 팁]"
예: "점심 식사 시간이에요 🍽️ 오후에는 단백질이 풍부한 메뉴가 좋아요. 네이버 지도에서 '주변 샐러드 볼'이나 '닭가슴살 도시락'을 검색해보세요."

**구조 가이드**
- 4-5문장 (약 120-150자)
- 인사 1문장 + 핵심 정보/리소스 2-3문장 + 추가 지원 제안 1문장
- 이모지는 시작 부분에 1개만
- 마크다운 제목 사용 금지
- 구체적인 검색어나 플랫폼명 포함

사용자에게 실질적 가치를 제공하되, 간결하게 전달하세요.`;

        console.log("[AI Resource Recommend] OpenAI 요청 시작");
        const completion = await openai.chat.completions.create({
            model: "gpt-5.1-2025-11-13",
            messages: [
                {
                    role: "system",
                    content: "당신은 전문적인 AI 비서입니다. 사용자에게 필요한 핵심 정보와 실질적인 리소스를 간결하게 제공하세요. 존댓말을 사용하고, 각 활동에 맞는 구체적인 검색어, 플랫폼, 실행 가능한 조언을 제시하세요. 장황하지 않되 가치 있는 내용을 담으세요."
                },
                {
                    role: "user",
                    content: prompt,
                },
            ],
            temperature: 0.7,
        });

        console.log("[AI Resource Recommend] OpenAI 응답 성공");
        const recommendation = completion.choices[0]?.message?.content || "리소스를 추천할 수 없습니다.";

        return NextResponse.json({
            recommendation,
            activity,
            category,
        });
    } catch (error: any) {
        console.error("[AI Resource Recommend] 에러 발생:", error);
        console.error("[AI Resource Recommend] 에러 상세:", error.message);
        console.error("[AI Resource Recommend] 에러 스택:", error.stack);
        return NextResponse.json(
            { error: "Failed to generate resource recommendation", details: error.message },
            { status: 500 }
        );
    }
}
