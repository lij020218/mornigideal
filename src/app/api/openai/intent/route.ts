...
- 기타는 chat.
    ...
    // 2) Act on intent
    if (parsed.intent === "schedule_update" && parsed.scheduleUpdate) {
        try {
            ...
            if (target && time) {
                ...
                const { error } = await supabaseAdmin.from("users").update({ profile }).eq("email", email);
                if (error) {
                    logger.error("[Intent] schedule update failed:", error);
                } else {
                    parsed.reply = parsed.reply || "일정이 업데이트되었습니다.";
                }
            }
        } catch (err) {
            logger.error("[Intent] schedule update error", err);
        }
    }
    ...
    if (parsed.intent === "briefing") {
        try {
            ...
            logger.error("[Intent] briefing fetch error", err);
        }
    }
