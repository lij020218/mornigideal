...
<body>
    <div class="container">
        ...
        <h2 id="status">Google 캘린더 연동 중...</h2>
    ...
    <script>
        (async function() {
            ...
            if (error) {
                ...
                document.getElementById('message').innerHTML = '<div class="error">Google 인증이 취소되었거나 실패했습니다: ' + error + '</div>';

                if (window.opener) {
                    window.opener.postMessage({
                        type: 'gcal-link-error',
                        error: error
                    }, '*');
                }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    document.getElementById('message').textContent = 'Google 캘린더가 연동되었습니다.';

                    if (window.opener && !window.opener.closed) {
                        window.opener.postMessage({
                            type: 'gcal-link-success',
                            data: data
                        }, '*');
                    } else {
