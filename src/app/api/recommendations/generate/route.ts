...
export const POST = withAuth(async (request: NextRequest, email: string) => {
  ...
  if (!job || !goal || !interests || interests.length === 0) {
    try {
      ...
      logger.error("[Recommendations] Failed to load profile:", error);
    }
  }
  ...
  try {
    ...
    logger.error("Failed to parse queries:", text);
  ...
  for (const q of queries.slice(0, 3)) {
    try {
      ...
      logger.error(`YouTube search error for "${q}":`, err);
    }
  }
  ...
  if (videos.length < 3) {
    ...
    for (const q of backupQueries) {
      ...
      try {
        ...
        logger.error(`YouTube backup search error for "${q}":`, err);
      }
    }
  }
